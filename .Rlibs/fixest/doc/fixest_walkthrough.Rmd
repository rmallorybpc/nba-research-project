...
```
Oh, now it works! By the way, note that if the same data set is used again and again, it is possible to set some characteristics globally, like the `panel.id` argument. This can be done with the `setFixest_estimation()` function:
```{r}
...
Let's have an example using the Fulton fish market data ([Graddy 1995](https://doi.org/10.2307/2556036)). 
This data consists of daily recordings of the price and quantity of fish sold in the NYC market.
We want to estimate quantity/price but are facing the well-known simultaneity problem from supply and demand being jointly determined. 
We will follow [Angrist, Grady, and Imbens (2000)](https://doi.org/10.1111/1467-937X.00141) and use the sea weather (measured in `speed2` and `wave2`) as a "supply shifter" to estimate the demand elasticity.
