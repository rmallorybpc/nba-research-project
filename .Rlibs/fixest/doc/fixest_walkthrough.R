...
## ---------------------------------------------------------------------------------------
est = feols(y ~ x1 | id, base_did, vcov = "cluster")
est_up = feols(y ~ x1 | id, base_did, vcov = "cluster", ssc = ssc(K.fixef = "full"))
est_down = feols(y ~ x1 | id, base_did, vcov = "cluster", 
                 ssc = ssc(K.adj = FALSE, G.adj = FALSE))
etable(est, est_up, est_down)

## ---------------------------------------------------------------------------------------
etable(est, vcov = list(~id, ~id + ssc(K.fixef = "full"), 
                        ~id + ssc(K.adj = FALSE, G.adj = FALSE)))

## ---------------------------------------------------------------------------------------
feols(y ~ x1 | id, base_did, iid ~ ssc(K.adj = FALSE))
feols(y ~ x1 | id, base_did, hetero ~ ssc(K.adj = FALSE))
...
## ---------------------------------------------------------------------------------------
est_iv_fe = feols(qty ~ t | day | price ~ speed2 + wave2, fulton)
est_iv_fe
...
iplot(res_twfe, res_sa20, sep = 0.5)
