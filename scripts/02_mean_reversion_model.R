...
#              quartile of pre_impact_overall) show systematically larger
...
#                 simple regression test of pre-signing impact -> MIS slope.
#
...
# population rather than an absolute impact threshold. Then mean MIS per quartile.
# ------------------------------------------------------------------------------
...
within_season_quartile <- function(df) {
  df %>%
    ...
    mutate(
      pre_impact_quartile = ntile(pre_impact_overall_mw, 4)
    ) %>%
    ungroup() %>%
    filter(!is.na(pre_impact_quartile)) %>%
    mutate(pre_impact_quartile_label = paste0("Q", pre_impact_quartile))
}

quartile_summary <- function(df) {
  df %>%
    group_by(pre_impact_quartile, pre_impact_quartile_label) %>%
    summarise(
      n                       = n(),
    ...
    arrange(pre_impact_quartile)
}

# ------------------------------------------------------------------------------
# Regression
# Simple OLS of MIS on pre-signing impact, with season fixed effects to absorb era
# effects. If mean reversion is present, the slope of pre_impact_overall_mw on mis_overall
# should be NEGATIVE: higher pre-signing performance predicts a larger drop.
# ------------------------------------------------------------------------------

regression_summary <- function(df) {
  fit <- lm(mis_overall ~ pre_impact_overall_mw + factor(contract_start_season),
            data = df)
  ts <- tidy(fit, conf.int = TRUE)
  ts %>%
    filter(term == "pre_impact_overall_mw") %>%
    mutate(model = "mis_overall ~ pre_impact + season_FE",
           n_obs = nobs(fit),
...
print_report <- function(qsum, rsum) {
  message("\n--- MEAN REVERSION ---")
  message("\nMean MIS by pre-signing impact quartile:")
  message(sprintf("  %-4s %-6s %-12s %-12s %-12s %s",
                  "Q", "n", "pre_impact", "post_impact", "MIS", "pct_neg"))
  pwalk(qsum, function(pre_impact_quartile, pre_impact_quartile_label, n,
    ...
    message(sprintf("  %-4s %-6d %+11.3f %+12.3f %+12.3f %.1f%%",
                    pre_impact_quartile_label, n,
                    mean_pre_impact_overall, mean_post_impact_overall,
                    mean_mis_overall, 100 * pct_negative))
  })

  message("\nRegression: mis_overall ~ pre_impact_overall_mw + season_FE")
  pwalk(rsum, function(model, term, estimate, std.error, statistic, p.value,
                       conf.low, conf.high, n_obs, r_squared) {
    message(sprintf("  slope on pre_impact: %+.3f (SE %.3f, p=%.3g, n=%d)",
